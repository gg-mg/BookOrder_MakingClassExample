...
public class BookOrder {
  ...
  //public double calcTotalPrice(double unitPrice, int numberOfBooks) we cannot code like this **********
  public double calcTotalPrice( notes...cannot put parameters here because we have access to private data, we cannot duplicate information)
  //  private data already. Adding formal parameter here is duplicate and wrong.
  ...
  public double calcSalePrice(double discountRate) {  //  discountRate is not a private data member of class BookOrder.
    double salePrice = this.calcTotalPrice(notes...this infront of calcTotalPrice is optional) * (1 - discountRate/100);
 ...
 //public double calcSalePrice(double discountRate)//we do not have double discountRate provided in the private data,that is why 
 //we define it here.
 
  //double salePrice = unit price * numberOfBooks  * (1 - discountRate/100);
  //double salePrice = this.calcTotalPrice(this infront of calcTotalPrice is optional) * (1 - discountRate/100);
 //public double calcTotalPrice(double unitPrice, int numberOfBooks) we cannot code like this **********